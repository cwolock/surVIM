estimate_AUC<- function(time,
  ...
  # calc_phi_01_othercross <- function(j){
  #   fx <- preds[j]
  #   varphi_x <- KM_IFs[j]
  #   Sx <- S_hat_k[j]
  #   int <- mean(-ifelse(preds > fx, 1, 0) * KM_IFs *Sx +
  #                 ifelse(preds <= fx, 1, 0) * KM_IFs*(1-Sx))
  #   return(int)
  # }
  ...
  # phi_01_othercross <- unlist(lapply(1:n, FUN = calc_phi_01_othercross))
