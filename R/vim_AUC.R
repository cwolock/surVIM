vim_AUC <- function(time,
  ...
  for(i in 1:length(landmark_times)) {
    ...
    CV_boot_var_ests<- rep(NA, length(unique(folds)))
    for (j in 1:length(unique(folds))){
      ...
      if (bootstrap){
        ...
        for (k in 1:B){
          ...
          V_0 <- estimate_AUC(time = time_holdout[boot_inds],
                              event = event_holdout[boot_inds],
                              approx_times = approx_times,
                              t0 = t0,
                              preds = f_hat[[j]][boot_inds,i],
                              preds_holdout = preds_holdout[boot_inds],
                              S_hat = S_hat[[j]][boot_inds,],
                              S_hat_holdout = S_hat_holdout[boot_inds,],
                              G_hat = G_hat[[j]][boot_inds,])
          V_0s <- estimate_AUC(time = time_holdout[boot_inds],
                               event = event_holdout[boot_inds],
                               approx_times = approx_times,
                               t0 = t0,
                               preds = fs_hat[[j]][boot_inds,i],
                               preds_holdout = preds_holdout_reduced[boot_inds],
                               S_hat = S_hat[[j]][boot_inds,],
                               S_hat_holdout = S_hat_holdout[boot_inds,],
                               G_hat = G_hat[[j]][boot_inds,])
