vim_AUC_newxfit <- function(time,
  ...
  for(i in 1:length(landmark_times)) {
    ...
    for (j in 1:length(unique(folds))){
      ...
      V_0 <- estimate_AUC_newxfit(time = time_holdout,
                          event = event_holdout,
                          ...
                          G_hat = G_hat[[j]])
      V_0s <- estimate_AUC_newxfit(time = time_holdout,
                           event = event_holdout,
                           approx_times = approx_times,
                           t0 = t0,
                           preds = fs_hat[[j]][,i],
                           preds_holdout = unlist(lapply(fs_hat, function(x) x[,i])[-j]),
                           S_hat = S_hat[[j]],
                           S_hat_holdout = do.call(rbind, S_hat[-j]),
                           G_hat = G_hat[[j]])
