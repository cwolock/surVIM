vim_cindex_newxfit <- function(time,
                       event,
                       approx_times,
                       tau,
                       f_hat,
  ...
  for (j in 1:length(unique(folds))){
    ...
    V_0 <- estimate_cindex_newxfit(time = time_holdout,
                           event = event_holdout,
                           ...
                           G_hat = G_hat[[j]])
    V_0s <- estimate_cindex_newxfit(time = time_holdout,
                            event = event_holdout,
                            approx_times = approx_times,
                            tau = tau,
                            preds = fs_hat[[j]],
                            preds_holdout = unlist(fs_hat[-j]),
                            S_hat = S_hat[[j]],
                            S_hat_holdout = do.call(rbind, S_hat[-j]),
                            G_hat = G_hat[[j]])
